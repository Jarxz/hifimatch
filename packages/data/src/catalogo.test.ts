...
test('conteo de equipos por categoría: 49 parlantes (46 reales + 3 genéricos) + 52 amplis (49 reales + 3 genéricos) + 34 streamers + 34 dacs + 3 cables = 172', () => {
  assert.equal(CATALOGO.parlantes.length, 49);
  assert.equal(CATALOGO.amplificadores.length, 52);
  assert.equal(CATALOGO.streamers.length, 34);
  assert.equal(CATALOGO.dacs.length, 34);
  assert.equal(CATALOGO.cables.length, 3);
  assert.equal(TODOS_LOS_EQUIPOS.length, 172);
});
