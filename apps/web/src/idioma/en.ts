...
export const en: Textos = {
  ...
  splash: {
    ...
    leadParrafo:
      'It evaluates the electrical coupling, power headroom and room acoustics between the gear you pick, with its own physical rules: each one declares its formula, its threshold, and where the data comes from.',
    cta: 'Analyze a system',
    ctaSecundaria: 'Explore the guide',
    ...
    proofReglasFormula: 'f = n · 343 / (2·L) — axial mode',
    proofAnalisis: 'analysis of verified gear',
    proofBusquedaOnlineDetalle: 'and via online search',
    proofAr: 'AR simulation',
    proofArDetalle: 'Augmented-reality reflections in your own room',
    novedadesTitulo: 'New features',
    novedadesBusqueda: 'Online equipment search',
    novedadesAr: 'AR reflection simulation',
    novedadesCerrarAria: 'Close',
    version: 'V20.08.26',
    recomendadoTitulo: 'The Match Recomendado',
    recomendadoIntro: 'Chosen by the engine itself from the whole catalog — changes every month.',
    recomendadoPositivoTitulo: 'Power and electrical coupling verified',
    bootPaso1: 'Loading catalog (170+ components)…',
    bootPaso2: 'Applying physical rules (Rayleigh room modes, Sabine/Eyring)…',
    bootPaso3: 'Separating physical law from editorial criterion…',
    bootProcesando: 'Processing…',
    bootListo: 'Done',
    bootSaltar: 'Skip intro',
    bootSistema: 'CADENA engine v2.4',
    bootInicializacion: 'Deterministic initialization',
    bootBadgeTop: 'No server',
    bootDiagnostico: 'System diagnostics',
    bootPieIzq: 'Deterministic architecture, no human judgment in the result',
    bootPieDer: 'Based on physics, measured specs',
    telemetriaEstado: 'Engine runs live, no server',
    capasEyebrow: "The project's structural rule",
    capasTitulo: 'Two layers of information. Zero confusion.',
    capasIntro:
      'Everything this site states belongs to one of two layers, and the layer is always declared — never mixed.',
    capasFisicaDesc:
      "Formula, threshold, and source, always declared. Every value comes from the manufacturer's spec sheet or an already-published independent measurement (Stereophile, Audio Science Review, Hi-Fi News), with its confidence level next to the number.",
    capasFisicaNota: 'Computed in your browser — no server, no human judgment in the result.',
    capasCriterioDesc: "The site's own design decisions, labeled as such, never mixed with a physical verdict.",
    capasCriterioEj1: 'Target peak SPL by listening level: 90 dB (moderate) · 100 dB (loud) · 105 dB (reference).',
    capasCriterioEj2: "Own 45° reference angle, declared separately from the 60° stereo-triangle convention.",
    capasCriterioEj3:
      'Two modes count as coinciding if they differ by less than an exact 1%, or if 2 or more pairs fall under 2% — the site\'s own threshold, not a published standard.',
  },
  ...
  config: {
    ...
    streamer: 'Streamer',
    dac: 'DAC',
    parlantesRol: 'Main transducer',
    amplificadorRol: 'Amplification stage',
    streamerRol: 'Streaming source',
    dacRol: 'Digital-to-analog converter',
    requerido: 'required',
    ...
    catalogoMarcaPlaceholder: 'Brand',
    catalogoModeloPlaceholder: 'Model',
    catalogoElegirMarcaPrimero: 'Choose a brand first',
    buscarMarcaPlaceholder: 'Brand',
    buscarModeloPlaceholder: 'Model',
    buscarBtnAria: 'Search',
    buscarEstadoBuscandoWeb: 'Searching the web…',
    buscarEstadoOffline: 'Offline (local file): automated search is not available.',
    buscarEstadoRegionRestringida: 'Automated search is not available in your region.',
    buscarEstadoCupoAgotado: "Today's search limit was reached — come back tomorrow.",
    ...
    // typed by the user.
    buscarSinResultadoIntro: "This equipment's specs couldn't be retrieved automatically. You can let the site know so it gets added to the catalog, or try the search again with a different name.",
    buscarManualIntroFuente: 'A streamer or DAC needs no required numeric field — you can use it as is, without output voltage or impedance.',
    buscarManualUsarSinDatos: 'Use without output data',
    buscarSolicitarAlta: 'Let the site know about this equipment',
    personalizarSala: 'Customize the room',
    resumenSala: (p) => `${p.ancho} × ${p.largo} × ${p.alto} m · ${p.muro} + ${p.piso}`,
    volumenBadge: (p) => `${p.m3} m³`,
    dimensionesTitulo: 'Room dimensions',
    relacion: (p) => `Ratio (height=1) 1 : ${p.w} : ${p.l}`,
    modoAxial: 'Axial mode of the largest dimension',
    ancho: 'Width (front)',
    largo: 'Length (depth)',
    alto: 'Height',
    ...
    nivelPico: (p: { db: string }): string => `${p.db} dB peak`,
    genero: 'Music genre',
