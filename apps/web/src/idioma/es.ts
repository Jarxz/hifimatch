...
export const es = {
  ...
  ar: {
    ...
    calibrandoAltura: 'Tocar la parte de arriba de esa misma esquina, donde la pared llega al techo.',
    ...
    medirAlturaBoton: 'Medir altura real',
    medicionAnchoHtml: (p: { ancho: string }): string => `Ancho medido en tu sala: <b>${p.ancho} m</b>.`,
    medicionAlturaHtml: (p: { alto: string }): string => `Alto medido en tu sala: <b>${p.alto} m</b>.`,
    medicionFueraDeRango: 'La medición no dio un valor creíble — se mantiene la medida ingresada en la web.',
    ...
    avisoWireframeAproximado:
      'La profundidad de la sala usa la medida ingresada en la web, no una medición real — si no coincide, el wireframe no calza exacto con la pared del fondo; no es un error de cálculo. El ancho (y el alto, si se mide) sí pueden ser reales — ver arriba.',
    ...
    // expone ninguna API que devuelva al sitio dónde/cómo el usuario
    // ancló el modelo, así que esta limitación se declara siempre, no
    // sólo una vez. Ver anclaje.ts (ANCLAJE_CANONICO) y escenaMalla.ts.
    quickLookAviso:
      'En iPhone, "Ver en AR" coloca un modelo de la sala calculada con las medidas ya ingresadas en la web — no mide la sala real como sí hace la versión de Android. La posición y la escala se ajustan con los gestos propios de AR Quick Look (arrastrar, pellizcar), sin calibración adicional de este sitio.',
    quickLookPreparando: 'Preparando el modelo…',
    quickLookError: 'No se pudo preparar el modelo para AR. Conviene volver al análisis e intentarlo de nuevo.',
  },
  ...
  motor: {
    ...
    triangulo: {
      ...
      texto: (p: { angulo: string; convencion: string; referencia: string }): string =>
        `El ángulo que subtienden los dos parlantes visto desde el punto de escucha es de <b>${p.angulo}°</b>. La disposición de referencia de este sitio apunta, por diseño, a <b>~${p.referencia}°</b> — un escenario más cerrado que la convención de triángulo equilátero estéreo (<b>${p.convencion}°</b>), citada aquí como el otro punto de referencia posible, no como un objetivo que este análisis tenga que cumplir. Ninguno de los dos ángulos es "incorrecto" — un escenario más angosto o más amplio es una preferencia legítima —; el aviso salta fuera de un rango declarado que contempla los dos valores, para que quedar fuera sea una decisión consciente y no un efecto no declarado de la disposición automática.`,
