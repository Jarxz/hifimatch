...
export function construirPlanoSvg(sala: Sala, disp: DisposicionSala, idioma: Idioma): string {
  ...
  s += `<text x="${X(cx)}" y="${coord(pad - 14, 0)}" fill="#6E6E75" font-size="10" letter-spacing="1.5" text-anchor="middle">${t.muroFrontal}</text>`;
  // dimensiones
  ...
  s += `<text x="${coord(parseFloat(X(0)) + 8, 1)}" y="${coord(parseFloat(Y(rpy)) - 7, 1)}" fill="#C7AD7C" font-size="9">${t.primeraReflexionCorta}</text>`;
  // parlantes
  ...
  s += `<text x="${X(cx)}" y="${coord(parseFloat(Y(ly)) + 20, 1)}" fill="#ECECEE" font-size="10" text-anchor="middle">${t.puntoDulce}</text>`;
